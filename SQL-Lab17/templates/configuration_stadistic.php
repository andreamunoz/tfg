...
<div class="container-tabla pt-4">
    ...
    <div class="row ">
        <div class="col-md-3 mb-3">
            ...
            $usuarios = new User();
            $nombre = $usuarios->getNombreApellidosUsuario($_SESSION['user']);
            $nombreCompleto = $nombre['nombre'] .' '. $nombre['apellidos'];
            $users = $usuarios->getNombreAlumnosAsociados($nombreCompleto);
            
            ?>
            <select class="custom-select" title="Selecciona" id="mostraruser" >
                <option value="" selected>Todos Alumnos</option>
                <option value="<?php echo $nombre['nombre'] .'-'. $nombre['apellidos'] ?>"><?php echo $nombre['nombre'] .' '. $nombre['apellidos'] .' (Profesor)'?></option>
                <?php while ($usuario = mysqli_fetch_array($users)) {
                    ...
                    echo '<option value='. $nombreCompu . ' >'.$usuario['nombre'].' '.$usuario['apellidos'].'</option>';
                }
                ?>
            </select>
        </div>
        ...
        <div class="offset-md-3 col-md-3 select-graficos p-0 text-right">
